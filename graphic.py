...
class Fantome:
    ...
    def deplacement_f(self) -> None:
        ...
        """
        déplacment_possible = []
        print(self.fantome.x,self.fantome.y,self.ancienx,self.ancieny)
        taille = cfg.taillecase
...
def génération_fantome(emplacement_prise:list, cords:list, registe_f:list) -> list:
    ...
    for prise in emplacement_prise:
        ...
        for case_autour in [(prise[0]-cfg.taillecase,prise[1]),(prise[0]+cfg.taillecase,prise[1]),(prise[0],prise[1]-cfg.taillecase),(prise[0],prise[1]+cfg.taillecase)]:
            if est_case_valide(case_autour[1]//cfg.taillecase,case_autour[0]//cfg.taillecase):
        ...
        if appariton_possible != []:
            ...
            cords[appariton[0]//cfg.taillecase][appariton[1]//cfg.taillecase] = "Fantome"
...
def action_des_fantomes(registre_f:list, carte:list, joueur:Bomber) -> None:
    ...
    for fantome in registre_f:
        if fantome.dois_attaquer_bomber(joueur):
            joueur.perte_vie()
        else:
            fantome.deplacement_f()
...
while joueur.en_vie():
    ...
    action_des_fantomes(registre_fantome, cords, joueur)
